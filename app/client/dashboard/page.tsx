...
export default function ClientDashboard() {
  ...
  const loadDashboardData = async () => {
    try {
      ...
      if (userData.role !== 'client_owner' && userData.role !== 'client_user') {
        router.push('/')
        return
      }
      setProfile(userData)
      ...
      <div className="max-w-7xl mx-auto space-y-4 sm:space-y-6 md:space-y-8">
        ...
        >
          <div className="min-w-0 flex-1">
            <h1 className="text-2xl sm:text-3xl md:text-4xl font-bold mb-1 sm:mb-2 gradient-text break-words">Client Dashboard</h1>
            <p className="text-sm sm:text-base text-muted-foreground truncate">Welcome, {profile.name || 'Client'}</p>
          </div>
          <Button
            onClick={() => router.push('/client/campaigns/builder')}
            variant="gradient"
